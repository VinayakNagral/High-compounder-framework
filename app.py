...
# Default results for graceful degradation when a layer fails
DEFAULT_ACCT = {
...
def fetch(ticker):
    ...
    try:
        ...
        return {"info": info, "fin": fin, "bs": bs, "cf": cf, "qfin": qfin, "ph": ph, "debug": debug}
    except Exception:
        return None
...
def run_layer1(sd, ticker):
    ...
    # Tiered growth: mature compounders get relaxed floor
    growth_floor = 15
...
def run_forensic(data):
    ...
    # CCC (simplified: latest + prior only)
    ccc_latest = None
    ccc_prior = None
    for d in sd:
        rev = d.get('revenue')
...
def gen_verdict(name, tier, l1, acct, cyc, val, ret, moat, is_bank):
    ...
    sents = []
    if tier == 'FULL':
        sents.append(f"{name} passes all seven layers with conviction.")
    elif tier == 'STANDARD':
...
def gen_layers(l1, acct, cyc, val, mom, ret, moat):
    ...
    if s is not None:
        if s >= 85:
            L.append(("Forensic accounting", "pass", f"Score {s}/100 | {nf} flags | CFO/PAT {fmt(acct.get('cum_cfo_pat'), '.2f')}x | CFO/EBITDA {fmt(acct.get('cum_cfo_ebitda'), '.2f')}x"))
        elif s >= 50:
    ...
    if mt2 >= 2:
        ...
        L.append(("Moat durability", st2, f"ROE>15% in {moat['years_above_15']}/{mt2} yrs ({moat['pct']}%) — {mc}"))
...
# ============================================================
# FULL ANALYSIS — try/except per layer
# ============================================================
def full_analysis(ticker):
    ...
    try:
        html = gen_scorecard(l1['name'], ticker, l1['sector'], l1['price'], tier, size, l1, acct, val, ret, moat, layers, verdict)
    except Exception:
        html = None
...
def analyse_with_funnel(ticker):
    ...
    return {'ticker': ticker.replace('.NS', ''), 'name': l1['name'], 'sector': l1['sector'],
            ...
            'fcf_yield': val.get('fcf_yield'), 'ccc': ccc_obj.get('ccc') if ccc_obj else None}, 'pass'
    ...
    for url in urls:
        try:
            ...
            if 'Symbol' in df.columns:
                return [s + ".NS" for s in df['Symbol'].tolist()][:max_stocks], url.split('/')[-1].replace('.csv', '')
        except Exception:
            continue
    fallback = [
        "RELIANCE","TCS","HDFCBANK","INFY","ICICIBANK","HINDUNILVR","BHARTIARTL","SBIN","ITC",
        "BAJFINANCE","KOTAKBANK","LT","AXISBANK","ASIANPAINT","MARUTI","SUNPHARMA","TITAN",
        "DMART","ULTRACEMCO","ONGC","NTPC","WIPRO","POWERGRID","HCLTECH","DIVISLAB",
        "BAJAJFINSV","TATAMOTORS","TATASTEEL","TECHM","JSWSTEEL","INDUSINDBK","HINDALCO",
        "APOLLOHOSP","DRREDDY","CIPLA","EICHERMOT","BPCL","GRASIM","SBILIFE","TATACONSUM",
        "HDFCLIFE","HEROMOTOCO","BRITANNIA","NESTLEIND","PIIND","LUPIN","DIXON","BSE","MCX",
        "KPITTECH","PERSISTENT","POLYCAB","COFORGE","CUMMINSIND","HDFCAMC","ICICIPRULI","VBL",
        "GODREJCP","MAZDOCK","BEL","HAL","SIEMENS","ABB","TRENT","ZOMATO","IRCTC","SBICARD",
        "MUTHOOTFIN","ASTRAL","AUROPHARMA","TORNTPHARM","IDFCFIRSTB","FEDERALBNK","CROMPTON",
        "VOLTAS","MPHASIS","LTIM","LTTS","TATAELXSI","DEEPAKNTR","ATUL","NAVINFLUOR",
        "BALKRISIND","LALPATHLAB","METROPOLIS","OBEROIRLTY","GODREJPROP","PRESTIGE","DLF",
        "PHOENIXLTD","MAXHEALTH","ZYDUSLIFE","GLENMARK","IPCALAB","LAURUSLABS","ABCAPITAL","CHOLAFIN",
    ]
    return [s + ".NS" for s in fallback][:max_stocks], 'hardcoded_fallback'
...
if page == "Single Stock":
    ...
    if go:
        ...
        card_data = [("Score", score_display), ("PE", fmt(l1['pe'], '.1f')), ("PEG", fmt(val.get('peg'), '.2f')),
                     ("ROE", fmt(l1['roe'], '.1f', '%')), ("FCF Yield", fmt(val.get('fcf_yield'), '.1f', '%')),
        ...
        with t4:
            ...
            chain = [
                ...
                ('Moat', moat.get('consistency', '?'), f"{moat.get('pct', 0)}%"),
...
# ============================
    ...
    if st.button("Run screen", type="primary", use_container_width=True):
        ...
        st.caption(f"Source: {source} | {len(tickers)} tickers loaded")
        results = []
        ...
        if failed_fetch > total * 0.2:
            st.warning(f"⚠ {failed_fetch} stocks failed to fetch — Yahoo Finance may be rate-limiting. Try fewer stocks or run again.")
        running = total
        ...
        df['rank'] = df['score'].fillna(0) * .20 + df['peg'].apply(peg_s) * .25 + df['roe'].fillna(0) * .10 + df['ret'].apply(mom_s) * .15 + df['cum_cfo'].apply(cfo_s) * .15 + df['moat'] * .15
        df = df.sort_values('rank', ascending=False).head(tn)
        # CSV export
        export_df = df[['ticker','name','sector','price','pe','peg','roe','roce','score','tier','size','sc','pc','cum_cfo','moat','fcf_yield','ret']].copy()
        export_df.columns = ['Ticker','Name','Sector','Price','PE','PEG','ROE%','ROCE%','Score','Tier','Size','Sales CAGR%','PAT CAGR%','CFO/PAT','Moat%','FCF Yield%','1Y Ret%']
        ...
        for idx, row in df.iterrows():
            ...
            with st.expander(f"#{rk} {row['ticker']} — {ic} {row['tier']} | Score {row['score']} | PEG {fmt(row['peg'], '.2f')}", expanded=rk <= 3):
                ...
                c5.metric("CCC", fmt(row.get('ccc'), '.0f', 'd') if is_valid(row.get('ccc')) else 'N/A')
...
# ============================
    ...
    st.markdown("""
**Layer 1 — Quantitative screen:** MCap > Rs 15,000 Cr, ROE > 15%, ROCE > 18%, D/E < 0.5, Sales & PAT CAGR > 15%. Mature compounders (MCap > Rs 50,000 Cr + ROE > 25%) get a relaxed 10% growth floor to capture steady compounders the standard 15% threshold would exclude.

**Layer 2 — Forensic accounting:** Four cumulative multi-year checks catch receivables stuffing, inventory bloat, and fake profits. Plus EBITDA margin trajectory and a payables stress detector that flags companies stretching supplier payments while cash conversion is weak.

**Layer 3 — Cash Conversion Cycle:** Days Receivable + Days Inventory - Days Payable. One-line summary with trend. Rising CCC means capital trapped in working capital. CCC improvement driven by exploding payables gets flagged separately when cumulative CFO/PAT is below 0.7x.

**Layer 4 — Moat durability:** *Established moat*: ROE > 15% in all available years. *Emerging moat*: latest two years above threshold with upward trajectory. *Weak/no moat*: inconsistent. Both established and emerging qualify for FULL tier.

**Layer 5 — Dual valuation:** PEG confirms growth justifies the PE. For turnaround stocks where PAT CAGR exceeds 2x Sales CAGR, revenue growth is used instead. FCF Yield confirms real cash generation relative to price. PE > 80 triggers the Adani Filter.

**Layer 6 — Earnings momentum:** Last 2 quarters QoQ EPS change catches fresh deterioration in an otherwise strong long-term story.

**Layer 7 — Cyclical ROE:** If ROE > 2x historical average and above 20%, cyclical peak. Use median ROE for any valuation work.

**Layer 8 — Position sizing v3:** Quality + moat + valuation + momentum -> FULL (12-15%), STANDARD (8-10%), HALF (4-6%), WATCH (0%). Each layer has a direct path to changing the tier.

---
**Structural exclusions:** Banking/NBFC (auto-detected), commodity producers (price-driven CAGR), newly listed (<2 years).

**Data integrity:** Shares cross-validated against market cap. EBIT uses 4 fallback field names. CAGR tries annual, then quarterly, then info dict. Every decision source logged in the Debug tab.
    """)
